...
-- backend/service/events.go (match_started, match_paused, match_resumed,
...
-- backend/service/events.go while retaining the legacy values for backward
...
ALTER TABLE match_events ADD CONSTRAINT match_events_event_type_check
    CHECK (event_type IN (
        -- Canonical CR-1 event types (see backend/service/events.go).
        'match_started',
        'match_paused',
        'match_resumed',
        'match_complete',
